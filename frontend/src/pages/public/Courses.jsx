...
import { FiBook, FiTrendingUp, FiAward, FiFilter } from "react-icons/fi";
...
const CoursesBooks = () => {
  ...
  // Wrap fetchBooks in useCallback with dependencies
  const fetchBooks = useCallback(async () => {
    try {
      ...
      const booksData = response.data.data.map((book) => ({
        ...
        image:
          book.cover_image && !book.cover_image.startsWith("blob:")
            ? book.cover_image
  ...
  useEffect(() => {
    ...
    fetchBooks();
    fetchCategories();
  }, [fetchBooks, fetchCategories]); // Add functions to dependency array

...
const FormationCard = ({
  ...
  const [isFavorite, setIsFavorite] = useState(
    favorites.includes(formation.id)
  );
  ...
  const formationData = {
    ...
    duration:
      ...
      (formation.duration_hours
        ? `${parseNumber(formation.duration_hours)} hours`
        : "Flexible"),
    ...
    highlights: Array.isArray(formation.highlights)
      ...
      : typeof formation.highlights === "string"
        ? formation.highlights.split(",").filter((h) => h.trim())
        : [],
    features: Array.isArray(formation.features)
      ? formation.features
      : typeof formation.features === "string"
        ? formation.features.split(",").filter((f) => f.trim())
        : [],
    prerequisites: Array.isArray(formation.prerequisites)
      ? formation.prerequisites
      : typeof formation.prerequisites === "string"
        ? formation.prerequisites.split(",").filter((p) => p.trim())
        : [],
    learning_objectives: Array.isArray(formation.learning_objectives)
      ? formation.learning_objectives
      : typeof formation.learning_objectives === "string"
        ? formation.learning_objectives.split(",").filter((lo) => lo.trim())
        : [],
    modules: Array.isArray(formation.modules) ? formation.modules : [],
    testimonials: Array.isArray(formation.testimonials)
      ? formation.testimonials
      : [],
    tags: Array.isArray(formation.tags)
      ? formation.tags
      : typeof formation.tags === "string"
        ? formation.tags.split(",").filter((t) => t.trim())
        : [],
  ...
  return (
    ...
    >
      {/* Card Glow Effect */}
      <div
        className={`absolute -inset-0.5 bg-gradient-to-r ${getLevelColor(formationData.level)} rounded-2xl blur-xl opacity-0 group-hover:opacity-30 transition-opacity duration-500`}
      />
      ...
      <div className="relative bg-gradient-to-br from-gray-800 to-gray-900 border border-gray-700/50 rounded-2xl overflow-hidden backdrop-blur-sm transition-all duration-500 group-hover:border-primary-500/30 group-hover:scale-[1.02]">
        ...
        <div className="relative h-56 overflow-hidden">
          ...
          <div className="absolute bottom-4 right-4">
            <div
              className={`relative px-3 py-1.5 bg-gradient-to-r ${getLevelColor(formationData.level)} text-white text-xs font-bold rounded-full backdrop-blur-sm`}
            >
        ...
        <div className="p-6">
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
              ...
              <div>
                ...
                <div className="text-white font-medium">
                  {formationData.duration_hours > 0
                    ? `${formationData.duration_hours} hrs`
                    : "Flexible"}
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-white font-medium">
                  {formationData.spots_left > 0 ? formationData.spots_left : 0}/
                  {formationData.max_participants > 0
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            ...
            {(formationData.features?.length > 3 ||
              formationData.highlights?.length > 1) && (
              <span className="px-2 py-1 bg-gray-800/30 text-gray-500 rounded-lg text-xs">
                +
                {(formationData.features?.length || 0) +
          ...
          <div className="flex items-center justify-between pt-4 border-t border-gray-700/50">
            <div className="flex flex-col">
              ...
              {formationData.original_price &&
                formationData.original_price > formationData.price && (
              ...
              {formationData.installment_price &&
                formationData.installment_price > 0 && (
          ...
          <div className="flex items-center justify-between mt-4 pt-4 border-t border-gray-700/30">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-gray-400">
                {formationData.rating?.toFixed(1) || "0.0"} (
                {formationData.reviews_count || 0})
              </span>
            </div>
      ...
      <div
        className={`absolute inset-0 bg-gradient-to-r ${getLevelColor(formationData.level)} rounded-2xl blur-xl -z-10 transition-all duration-500 ${
          ...
          <div className="absolute -top-2 -right-2 z-20">
            <div className="relative">
              ...
              <div className="relative px-3 py-1 bg-gradient-to-r from-red-600 to-pink-600 text-white text-xs font-bold rounded-full backdrop-blur-sm">
                Save{" "}
                {Math.round(
                  (1 - formationData.price / formationData.original_price) * 100
                )}
                %
              </div>
            </div>

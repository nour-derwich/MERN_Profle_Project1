...
const StatusBadge = ({ 
  ...
  const getSpotsColor = (spots) => {
    if (spots === null || spots === undefined) return 'from-gray-600 to-gray-800';
  ...
  return (
    <div className={`relative inline-block ${className}`}>
      ...
      >
        {/* Glow Effect */}
        <div className={`absolute -inset-0.5 bg-gradient-to-r ${config.glow} rounded-full blur-md opacity-0 group-hover:opacity-50 transition-opacity duration-300`} />

        {/* Badge Container */}
        <div className={`relative bg-gradient-to-r ${config.gradient} ${sizeProps.padding} rounded-full backdrop-blur-sm border border-white/10 hover:border-white/20 transition-all duration-300`}>
          <div className="flex items-center gap-2">
            ...
            {/* Text */}
            <span className={`${sizeProps.text} font-bold text-white whitespace-nowrap`}>
              {config.text}
            </span>
            ...
            {timeLeft && showCountdown && (
              <div className="flex items-center gap-1 ml-2 pl-2 border-l border-white/30">
                ...
                <span className={`${sizeProps.countdown} font-medium text-white/90`}>
                  {timeLeft}
                </span>
              </div>
            )}
          ...
          {/* Interactive Arrow */}
          {interactive && (
            <FiChevronRight className={`absolute -right-2 top-1/2 -translate-y-1/2 text-white opacity-0 group-hover:opacity-100 transition-all duration-300 ${isHovered ? 'translate-x-1' : ''}`} />
          )}
        </div>
        ...
        {isExpanded && spotsLeft !== null && (
          <>
            ...
            >
              <div className="relative">
                {/* Card Glow */}
                <div className={`absolute -inset-0.5 bg-gradient-to-r ${getSpotsColor(spotsLeft)} rounded-xl blur-md opacity-30`} />
                ...
                <div className="relative bg-gradient-to-br from-gray-800 to-gray-900 border border-gray-700/50 rounded-xl p-4 backdrop-blur-sm">
                  ...
                  <div className="flex items-center gap-3 mb-4">
                    <div className={`p-2 bg-gradient-to-r ${getSpotsColor(spotsLeft)} rounded-lg`}>
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      <span className={`text-lg font-bold bg-gradient-to-r ${getSpotsColor(spotsLeft)} bg-clip-text text-transparent`}>
                        {spotsLeft}
                      </span>
                    </div>
                    ...
                    <div className="relative h-2 bg-gray-700 rounded-full overflow-hidden">
                      <motion.div
                        initial={{ width: 0 }}
                        animate={{ width: `${Math.min(100, (spotsLeft / 50) * 100)}%` }}
                    ...
                    <div className="text-xs text-gray-500 text-center pt-2">
                      {spotsLeft <= 5 ? 'Almost full!' : spotsLeft <= 15 ? 'Limited availability' : 'Good availability'}
                    </div>
                  </div>
